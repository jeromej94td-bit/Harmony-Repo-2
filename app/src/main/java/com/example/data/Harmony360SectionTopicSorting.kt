...
object Harmony360SectionTopicSorting {
    ...
    fun apply(pack: GenPack): GenPack {
        val topic = topicByPackId[pack.id] ?: run {
            if ("h360_section_12_kommunikation_konflikte" in pack.tags) {
                "beziehung"
            } else {
                return pack
            }
    ...
    fun apply(pack: QuestionPack): QuestionPack {
        val topic = topicByPackId[pack.id] ?: run {
            if (pack.tags.contains("h360_section_12_kommunikation_konflikte")) {
                "beziehung"
            } else {
                return pack
            }
