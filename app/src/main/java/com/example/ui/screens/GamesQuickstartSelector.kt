...
internal data class GamesQuickstartPool(
    ...
    fun pick(nextInt: (Int) -> Int = { bound -> Random.nextInt(bound) }): GamesQuickstartCandidate? {
        ...
        // Kategorie zuerst wählen: Große Kategorien sollen nicht automatisch häufiger erscheinen,
        // nur weil sie mehr Spiele enthalten.
...
private fun isQuickstartCompatible(pack: QuestionPack): Boolean {
    // Diese Experiences besitzen bewusst eine eigene Navigation bzw. Fortschrittslogik.
    // Quickstart greift nur auf Packs zurück, deren bestehender Runner zuverlässig bei
    // der ersten noch offenen Frage fortsetzt.
