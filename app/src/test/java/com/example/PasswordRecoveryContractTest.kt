...
class PasswordRecoveryContractTest {
    ...
    fun `forgot password completes recovery inside the app`() {
        ...
        assertTrue(supabase.contains("scheme = PASSWORD_RECOVERY_SCHEME"))
        assertTrue(supabase.contains("host = PASSWORD_RECOVERY_HOST"))
        assertTrue(manifest.contains(".PasswordRecoveryActivity"))
        assertTrue(manifest.contains("android:scheme=\"com.aistudio.harmony.couples.xqvz\""))
        assertTrue(manifest.contains("android:host=\"password-recovery\""))

        val recovery = source("app/src/main/java/com/example/PasswordRecoveryActivity.kt")
