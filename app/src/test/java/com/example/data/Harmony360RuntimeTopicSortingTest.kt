...
class Harmony360RuntimeTopicSortingTest {
    ...
    @Test
    fun `accepted relationship overflow moves appear in runtime`() {
        val runtimeById = GeneratedContentRegistry.PACKS.associateBy { it.id }

        expectedGeneratedTopics.forEach { (id, expectedTopic) ->
            println("Testing $id"); assertEquals("runtime pack $id", expectedTopic, runtimeById.getValue(id).topic)
        }
    }

    @Test
    fun `stale custom copies cannot move sorted packs back into Beziehung`() {
