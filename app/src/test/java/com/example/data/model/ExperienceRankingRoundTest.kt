...
class ExperienceRankingRoundTest {
    ...
    @Test
    fun `stable id ranking selection preserves existing label answer format`() {
        val round = round()
        val ids = listOf("c", "a", "b")
        val encoded = ExperienceRankingSelectionCodec.encode(round, ids)

        assertEquals(
            RankingAnswerCodec.encode(listOf("Ort", "Nähe", "Überraschung")),
            encoded
        )
    ...
    fun `proposal ranking adapts without changing ids labels prompt or item count`() {
        ...
        assertEquals(
            ProposalPriorityRanking.priorities.map(ProposalPriority::id),
            generic.items.map(ExperienceRankingItem::id)
        )
        assertEquals(
            ProposalPriorityRanking.priorities.map(ProposalPriority::label),
            generic.items.map(ExperienceRankingItem::label)
        )
