...
export function createDeleteAccountHandler({ fetchImpl = fetch, getServiceRole, getSupabaseUrl } = {}) {
  return async (request) => {
    ...
    // Determine the caller from the JWT. Never accept a user id from Android.
    ...
    // Break the active two-person relationship first so the remaining account is immediately unpaired.
