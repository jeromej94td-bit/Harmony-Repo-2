...
1. "Wofür nehmt ihr euch spontan einen freien Tag?" – Optionen: "Kleiner Roadtrip", "Zeit nur für uns", "Etwas Neues erleben", "Ein Herzensprojekt starten".
2. "Welche Überraschung würde euch wirklich freuen?" – Optionen: "Ein geplanter Abend", "Ein spontaner Ausflug", "Eine persönliche Geste", "Ein gemeinsames Upgrade".
3. "Welche gemeinsame Idee sollte endlich passieren?" – Optionen: "Unser nächster Kurztrip", "Ein neues Ritual", "Ein Projekt zu zweit", "Ein mutiger erster Schritt".

In jedem Kapitel wählen Person A und Person B nacheinander über zwei leuchtende, nicht beschriftete Siegel. Die jeweils gewählte Antwort bleibt verdeckt; der Bildschirm zeigt nur eine neutrale Bestätigung und die Übergabe an die andere Person. Nach Kapitel eins und zwei wird ohne Reveal in das nächste Kapitel gewechselt. Im dritten Kapitel erscheint nach der zweiten Wahl eine aufklappende Plan-Karte.
...
`SecretPlanAnswerCodec` speichert die zwei privaten Antworten eines Kapitels in einem versionierten String. `SecretPlanProgress` filtert ausschließlich Antworten aus `geheimer_plan`, dekodiert sie pro Frageindex und baut für Kapitel drei die drei Plan-Zeilen. Die Logik ist reine Kotlin-Logik und wird ohne Datenbank, Netzwerk oder KI getestet.
...
1. Der neue Pack ist sichtbar, enthält genau drei Kapitel und alle festgelegten Optionen.
2. Der Codec dekodiert ausschließlich gültige `secret-plan-v1:`-Werte und verwirft fremde oder unvollständige Werte.
